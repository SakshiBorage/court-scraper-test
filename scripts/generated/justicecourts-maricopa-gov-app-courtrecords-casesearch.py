...
def scrape(page: Page, params: dict) -> dict:
    ...
    result['courtName'] = result['courtName'] or _rx(raw, 'Location:\\s*(.+?)\\s+Justice Court')
    result['countyName'] = result['countyName'] or _rx(raw, 'Justice Courts of (.+?) County')
    result['defendantFullName'] = result['defendantFullName'] or _rx(raw, 'Defendant\\s*Party Name\\s*Eric Francis Smith')
    result['chargedOffenses'] = result['chargedOffenses'] or _rx_all(raw, 'ARSCode\\s*28-737A \\(T1\\)\\s*Description\\s*HOV LANE W/O TWO IN VEHICLE\\s*Crime Date\\s*3/6/2007 5:15 PM\\s*Disposition Code\\s*58\\s*Date\\s*3/29/2007\\s*Disposition\\s*Failure to appear on civil traffic offense; default judgement entered; suspension of operating privilege ordered.')
